    ...
    for c in 0..ncol {
      for d in dirs {
        ...
        for i in 0..4 {
          ...
          if r_pos < 0 || r_pos >= nrow as i32 || c_pos < 0 || c_pos >= ncol as i32
...
fn part2(file: &str) -> io::Result<u64> {
  ...
  for r in 0..nrow {
    for c in 0..ncol {
      let center = r*ncol + c;
      if data[center] != 'A' || (r as i32)-1 < 0 || r+1 >= nrow || (c as i32)-1 < 0 || c+1 >= ncol {
        continue;
      }
      ...
      if ((data[top_left] == 'M' && data[bottom_right] == 'S') || (data[top_left] == 'S' && data[bottom_right] == 'M'))
      && ((data[top_right] == 'M' && data[bottom_left] == 'S') || (data[top_right] == 'S' && data[bottom_left] == 'M')) {
        result += 1;
      }
    }
  }
