  ...
  loop {
    if let Some((new_row, new_col, new_dir)) = advance(cur_row, cur_col, cur_dir, nrow, ncol, &map) {
      ...
      if map[idx] == '.' {
        ...
        loop {
          if let Some((new_row2, new_col2, new_dir2)) = advance(cur_row2, cur_col2, cur_dir2, nrow, ncol, &map2) {
...
fn advance(cur_row: i32, cur_col: i32, cur_dir: char, nrow: usize, ncol: usize, map: &Vec<char>) -> Option<(i32, i32, char)> {
